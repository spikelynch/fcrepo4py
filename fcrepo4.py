...
"""
    ...
    def add_container(self, uri, metadata, slug=None, path=None, force=False):
        ...
        """
        if path:
            return self._put_resource(uri, metadata, None, path, force)
        else:
            return self._post_resource(uri, metadata, None, slug)
    ...
    def add_binary(self, uri, source, slug=None, path=None, force=None):
        ...
        if path:
            return self._put_binary(uri, source, path, force)
        else:
            return self._post_container(uri, source, slug)

        
    def _put_resource(self, uri, metadata, data, path, force):
        """Internal method: creates a resource (container or binary) on
        a given path.
        """
        headers = {}
        if data:
            ( mtype, data ) = self._handle_data(data)
            headers = { 'Content-Type': mtype }
            metadata = None
        else:
            data = metadata.serialize(format=RDF_MIME)
            headers = { 'Content-Type': RDF_MIME }
        newpath = uri + path
        self._ensure_path(newpath, force)
        response = self.api(newpath, method='PUT', headers=headers, data=data)
        if response.status_code == requests.codes.created:
            uri = response.text
            if metadata:
                return Resource(self, uri, metadata=metadata)
            else:
                return Resource(self, uri) # FIXME (data = data?)
        else:
    ...
    def _post_resource(self, uri, metadata, data, slug=None):
        """Internal method to add an autogenerated or slugged resource."""
        headers = {}
        if data:
            ( mtype, data ) = self._handle_data(data)
            headers = { 'Content-Type': mtype }
        ...
        if slug:
            headers['Slug'] = slug
        response = self.api(uri, method='POST', headers=headers, data=data)
        if response.status_code == requests.codes.created:
            uri = response.text
            if metadata:
                return Resource(self, uri, metadata=metadata)
            else:
                return Resource(self, uri) #FIXME
        else:
...
"""
    ...
    def add_binary(self, metadata, source, slug=None, path=None, force=False):
        """Add a new binary object to this resource.

        Parameters:
        metadata ([ (p, o) ]) -- a Graph
        ...
        """
        return self.repo.add_binary(self.uri, metadata, source, slug=slug, path=path, force=force)
